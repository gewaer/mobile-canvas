...
class SwipeBackRow extends Component<Props, State> {
  ...
  render() {
    const { style, canEdit, buttonWidth, onDeletePress } = this.props;

    return (
      <View style={style}>
        <TouchableOpacity
          style={[{ width: buttonWidth }, Stylesheet.editButton]}
        >
          <Icon
            type="MaterialIcons"
            name="mode-edit"
            color={colors.normalWhite}
            style={[Stylesheet.icon, { fontSize: 30 }]}
          />
        </TouchableOpacity>
        <TouchableOpacity
          style={[
        ...
        >
          <Icon
            ...
            name="trash"
            color={colors.normalWhite}
            style={[Stylesheet.icon, { fontSize: 22 }]}
          />
        </TouchableOpacity>
      </View>
    );
